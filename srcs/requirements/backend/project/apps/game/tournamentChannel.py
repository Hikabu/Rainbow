...
class TournamentChannel():
	...
	async def confirm_payment(self, consumer, alias, wallet_address):
		global entry_price, max_players

		self.registered[consumer.user_id] = {"user" : consumer, "alias":alias, "wallet_address": wallet_address}
	...
	async def start(self):
		global waitTime, min_players, _token, controlsTime, entry_price
		...
		if len(self.players) == 1:
			...
			await self.pay_user(self.now_waiting[0], entry_price)
		...
		#start tournament
		logger.debug(f"players at start: {self.players}")
		self.status = "active"
	...
	async def matchmake(self):
		...
		while len(self.now_waiting) >= 2:
			await self.start_remote_game(self.now_waiting.pop(0), self.now_waiting.pop(0))
	...
	async def start_remote_game(self, player1_id, player2_id):
		...
		logger.info(f"new game with id: {player1_id}, {player2_id}")
		logger.info(f"new game with alias: {self.players[player1_id]["alias"]}, {self.players[player2_id]["alias"]}")
		...
		if player1_id in self.players and player2_id in self.players: 
			...
			logger.debug("split prize, 2 disconnections final round")
